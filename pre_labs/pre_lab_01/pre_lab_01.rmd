...
chr is the data type for id, and dbl is the data type for seats
...
**Answer** The first line of code is sorting within the umd courses, the second line of code is grouping all of those courses by each term, the third line is summarising the fourth line which is counting the number of math classes for each term without having to show all the math involved.
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** There is one new line of code under that is arranging by descenting classes. The results were affected because the code is now going from the highest number of courses offerred to the least.
...
**Answer** The output changed because instead of the term being the defining factor, the department it is in is now the defining factor.
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** The output has changed because now the classes are organized in descending order by the amount of seats availible in the class for the whole semester. This affected the result because the department is no longer the deciding factor in the data.
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer** What has changed is that more data is being added around the the top terms, because the two top terms are the same. What they have in common know is they both have relatively similar means.
...
**Answer** Compared to the previous code, what has changed is now the total number of seats is not sorted by the term it is sorted by the department. This affected the result because now terms are not the ruling factor, what department has the most seats per term is. That is how this data is sorted,from most to least total seats.
...
1.  I would like to ask this data what percent of those total seats are filled each term according to each department?

2. I would also like to ask how much total enrolment do the departments with the lowest total number of seats have?

3. My last question is, are any departents in danger of falling apart due to lack of seats or enrollemnt per term?
