...
The dataframe explains each statee's spending per student and how that correlates to the change in population and the gdp the year after they voted for a certain candidate in the 2020 election. It attempts to show a correlation between all of these factors. 
...
The plot that I see has all of the states respresented as dots on the graph, and shows the relationship between gdp and the population growth. The pattern I see is that states with a less than one percent growth in population tended to have a consistent improvement in GDP of about 5% in 2021.
...
The plot here compares the gdp with the amount spent on each pupil. I think there is less of a correlation between the two because the data has a slighlty downward trend, but the dots appear to be all over the place. But the states that spend the least per pupil tended to see a growth in GDP of around 5% in 2021.
```{r}
...
The table shows the strength of relationships between variables using the correlation coefficient. The relationship between the gdp and per pupil is the least strong, while the connection between gdp and population change is somewhat strong and positive. But the strongest connection in terms of correlation with r is between per pupil spending and population change in 2021, it is a negative medium correlation.
...
This model looks quite ugly, because it is just lines of text and not a graph. I see how the correlation was calculated using the confidence interval and the t test to find the middle of the two at the opposite ends of the interval. The p value is also quite low in this case, which is usually a good sign of correlation.
...
**Task** Run the following code. Describe the model output. What do you see? **Answer**
The model output shows how the correlation is not very high and not very close at all. It shows a very low negative t value, and a much higher p value than the last set of data. It also has two ends of the confidence interval spectrum that are positive on one end and negative on the other end. 
...
It compares the numbers we've been looking at in each state and compares them based on which states were won by each candidate in 2020. It is pretty obvious to see that Biden's states had a bigger growth in gdp and more spending per pupil in the year after the election. However states that trump won had a much bigger population growth than those won by Biden. Like the population growth disparity is the greatest out of any of the variables being compared.
...
I see the numbers from population being compared in a t test. The t number and the p value are both extremely low in this case which may mean there is not a lot of legitimate data to be reported here because the two numbers of population growth are nto comparable to each other as much as they look.
...
**Task** Run the following code. Describe the output. What do you see? **Answer**
The output here sees the two per pupil data being compared. It shows an extremely high confidence interval, the highgest we've seen out of any data so far. And also the p value is extremely low, and the t value is very high in this case. This could be good for this set of results, making them more legit.
