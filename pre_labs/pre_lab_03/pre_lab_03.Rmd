...
The first line of the red warning message says "One or more parsing issues, call problems on your data frame for details."
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer There are some categories that are listed as N/A which could be potentially extremely problematic because you don't know where the money is coming from.**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer I see data that describes each specific donation, where it came form, who it came from, and when it was placed and for how much.**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer The function changes the headers by making them lowercase.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? There are 50 rows of duplicates listed here, meaning that 25 duplicates are possible.
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer What changed is the results that had the nine digit zip codes now have an extra column that just contains the first five digits of that nine digit zip code.**
...
**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board? Does that zip code make sense to you? The zip code that has accepted the most from the Maryland Tourism Board is 20772, this does shock me a bit because I feel like my code is incorrect.

```{r}
cleaned_md_grants_loans |>
         group_by("Commerce/Maryland Tourism Board") |>
