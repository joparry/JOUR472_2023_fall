...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer A problem that I see here that is preventing proper grouping is that some of these groups seem to be the same group but with one letter misspelled, and all of them start with the letter w so it's not an efficient way to filter the groups. Also all the uppercase groups are put first.**
...
Two changes that the refinr package made in the first 30 rows that I noticed were first changing Melwood Horticultural Training Center, Inc. to Melwood Horticultural Training Center, and the second change I noticed was changing Charles County Arts Alliance Inc to CHARLES COUNTY ARTS ALLIANCE.
...
The first change that I noticed the refinr function made was changing WASHINTON ADVENTIST UNIVERSITY to Washington Adventist University. There are no other changes in the first 30 results.
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? No the changes that were made do not vary substantially from the previous results at all.
