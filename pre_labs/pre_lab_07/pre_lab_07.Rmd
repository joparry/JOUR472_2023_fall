...
```{r echo=FALSE}
census_api_key("75aa1a4a0cc0f3bcb9029f4aa194c59ec9020cc6", install = TRUE, overwrite = TRUE)
```
...
**Task** Run the following codeblock to calculate the percentage change and put the state with the largest percentage growth below. **Answer Nevada was the state with the largest percent growth.**
...
**Task** Run the following codeblock to find the Maryland county with the highest median household income and put the answer below. **Answer Howard County had the highest median household income in the state of Maryland.**
...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? It should read 2017-2021 Maryland Community Survey.
...
Is the difference between Calvert and Montgomery significant? No, it is not significant.

Is the difference between Howard and everyone else significant? **Answer Yes the difference between Howard and everyone else is significant.**
...
**Task** Let's look at the average household income for high schools that have 60% or more graduates immediately enrolling in college, and then look at those with less than 40%. Write a sentence describing the results. **Answer The average household income for the counties with more enrolling in college after high school was six figures a year, while the median household income for the lower college enrolled percentage was close to half of the previous group, despite the counties outnumbering the higher median household income counties.**
...
**Task** What is another question you could ask of this data? **Answer You could ask how many people are working in an average household of the counties that have less than 40% enrolling in college.**
