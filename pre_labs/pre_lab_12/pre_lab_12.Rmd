...
The dataframe that is outputed is just a record of press releases about Senator Ben Cardin from the last few years.
...
The unique function just simplifies the original list to cancel out multiples of the same word in the list.
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** The output is that it singles out unique words in the press release titles in the text column and orders them in a list.
...
I don't think this graphic is very interesting at all, the words used the most are just common english words used in most sentences, it doesn't really tell us that much about the senator besides the fact he's from Maryland.
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** This graphic is alot more interesting than before because it shows words that are much more important for senators. Helath was the most used, with support and project coming in second and third.
...
The graphic just shows the amount of time each of the words were used, and the percent of the time in which they were used in a press release relating to the senator. Act and national being the top two words, which isn't that surprising.
...
**Task** Run the following code and describe the results. **Answer**
The code singles out the top 10 two word combinations that were used together in a press release form the senator. One name was used in the top 10, which was John Sarbanes. Two cities made the list, Baltimore City and Chesapeake Bay. Covid 19 was number one, shocks no one, because of the year.
...
**Task** Run the following code and describe the results. **Answer**
It shows the same results as before, but for 2022. The results are different, because Covid 19 is not number one anymore, it is now down to number four on the list. Five of the phrases relate to locations in Maryland. Project location and amount included are numbers one and two, which for someone in congress isn't crazy, shows he is trying to get stuff done.
...
**Task** Run the following code and describe the results. **Answer**
The results here are the same thing as before but for 2023. Thye number one phrase is human rights, makes sense because the Israel Palestine War. In an odd twist, oral health is at number five for some reason, with supreme court coming right after at number six.
...
The results are the most used bing words in each year, and what the sentiment behind them is. Support is the highest used bing word in each year, and it has a positive statement. Then most used negative owrd over all three datasets is critical, which is used in 2022 301 times. I think that the word work being called positive seems a bit incorrect, because that word doesn't always mean something encouraging, like telling people they need to work isn't something positive. 
