...
In the answer space below I see a bunch of descriptions of the file and then a description of the columns and rows in the data. The columns that exist in the data are OBJECTID, ZIPCODE2, ZIPNAME, EXISTING, ZIPCODE1, Shape_Leng, Shape_Area, and geometry.
...
The information in the space below is a description of each zip code in the state of Maryland based on the size and length and geometric size of the area code to make an accurate representation of each zip.
...
The output below is a geographically accurate map of the state of Maryland that is colored black and white with a lot of grey in the middle.
...
The packages are installed and the message says that there is an error in install.packages, because it is updating the loaded packages.
...
The output in the space below shows counties and different aspects of them like the area of land and the area of water in each county.
...
The output when doing this is a chart that labels all of the 24 counties in the state as 24, and it has each of the county descriptions well organized from the data that we previously saw.
...
The output in the space below looks like the previous map we saw, but the gaps in the areas are much bigger because we are dealing with counties rather than zip codes.
...
The output below shows both of the maps combined and with a lot more areas of darker spaces to represent population density I believe. It just is a lot more of a dense map than before and it looks pretty factual and complex.
...
The output is a two collumn dataset that shows the amount of foreclosures per each zip code in Maryland in March of 2023.
...
The output below just adds the column of the march foreclosures to the zip code data we had previously by using left_join.
...
The map below uses the county and zip data from the previous maps and puts in the foreclosure data into the map via location to see where the hot spots of foreclosures are. The Northeast and the Southwest of the state have clusters with a lot of foreclosures, especially near the DMV area.
...
The output in the space below only talks about how to install the API key for future use, we are just installing libraries here.
...
The output below are two datasets, one of which has the notices of foreclosure per each county with the amount of them for each, and we also have the estimated population per county in another dataset.
...
The output below just adds the geometry of each county on to the county data we just previously looked at. It spits out a new raw look of the data below in the output under the code, with the new column included.
...
The output below just shows the two datasets side by side in their updated form compared to when we were just looking at them to start.
...
The output below just shows the Maryland county population data before, but with some of the column names being cleaned so that you can join this data with the foreclosure data.
...
The output in the dataset below just adds the county and population data onto the data of the amount of foreclosure notices in Maryland counties.
...
The output in the space below shows that the populations have changed for the counties from the previous data, and it adds a column with the rates of foreclosures per 1000 in the population in each county as well.
...
The output below shows the map with a blue scaled color code to highlight rates of foreclosures per each county, with the names of each county in black font right on top of where the county is located on the map in regular black font.
...
The new map takes away the names of the counties on the map from before and introduces a completely different color code to display the foreclosure rates. This time the map looks more clear because the different colors for each part of the rate make the rates more easily distinguishable from each other.
...
The new map introduces lighter but different colors to display high rates of foreclosure, and darker but different colors to display low rates of foreclosure. The regional pattern is that the Southern part of the state has much higher rates of foreclosure than the Northern part of Maryland, with the most Southwestern point of the state peaking in foreclosure notice rate.
