...
The differences between the two bar charts is that the highest total number of calls is a lot fewer on this bar chart than the previous one. And the lowest number of calls in Baltimore was 200, while in PG county it is around 90 total calls per the least active month.
...
**A2. There is a massive spike increasing near the 30th week of the year, and then a massive decrease in calls towards the end of the weeks. Besides this, the graph is mostly consisting of regular up and down trends.**
...
**A3. I would say with the way that data looks on the scatter plot, the median income being low does have some indication of an increase in that area of 911 calls. I think this means that society is working the way its supposed to, not protecting the disenfranchised. I would be pretty frank and honest in the way I report this, there is literally a peak in the data right at 75,000 dollars a year. I mean come on now.**
...
colnames(pg_calls_zipcode) <- c("zipcode" , "total_calls")

md_median_income_zip <- get_acs(geography = 'zcta', variables = 'B19013_001', state = 'MD', year = 2019)

combined_ish <- pg_calls_zipcode |>
  left_join(md_median_income_zip , by = c("zipcode" = "GEOID"))
```

```{r}

scatterplot <- ggplot(data = combined_ish, aes(x = estimate, y = total_calls, label = zipcode)) +
  geom_point(na.rm = TRUE) +
  geom_text(check_overlap = TRUE, angle = 45, hjust = -.1, size = 2)+ 
  labs(
