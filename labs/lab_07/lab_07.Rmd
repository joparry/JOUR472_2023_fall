...
```{r echo=FALSE}
census_api_key("75aa1a4a0cc0f3bcb9029f4aa194c59ec9020cc6", install = TRUE)
```
...
In Maryland, only Talbot County and Worcester County have a median age that is above 50 years old.
...
**A3 20877 was the zip code with the highest rate of calls, and 20707 was the zip code with the lowest rate of calls.**

```{r}
moco_911_calls <- read_csv('data/montgomery_2022_overdoses.csv') |>
  mutate(zip = as.character(zip))

moco_911_calls_per_zipcode <- moco_911_calls |> group_by(zip) |> summarize(count = n())
...
moco_join_pop_calls <- moco_911_calls_per_zipcode |> inner_join(md_pop_zcta, by=c('zip' = 'GEOID'))

moco_join_pop_calls <- moco_join_pop_calls |> mutate(rate_per_1k = (count/estimate)*1000) |>
...
**A4 The biggest difference from a demographic standpoint between zip codes 20877 and 20707 is the diversity. 20877, which has the highest rate, has a demographic makeup of 21% White, 18% Black, 16% Asian, and 42% Hispanic population, while 20707 has a makeup of 22% White, 45% Black, 11% Asian, and 16% Asian. While both zip codes have an average age between 37 and 38, I feel like age isn't the story here. I feel like the potential story here could be why do people potentially in populations with more than 40% of the Black population have such few calls. That is if that number starts to be seen as a trend in the data.**
...
**A5 I got the same two zip codes that I got in Q3. I think that this gives me a lot more confidence in Q3 because I have now seen the same numbers that were gathered in two different ways.**
